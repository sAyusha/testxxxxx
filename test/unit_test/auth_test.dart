...
import 'package:swapreads/features/auth/domain/entity/auth_entity.dart';
...
void main() {
  ...
  test('login test with invalid username and password', () async {
    ...
    await container
        .read(authViewModelProvider.notifier)
        .loginUser(context, 'nischal', "nischal");

    final authState = container.read(authViewModelProvider);
    expect(authState.error, 'Invalid'); // Updated assertion
  });
  ...
  test('register test with invalid user information', () async {
    ...
    await container.read(authViewModelProvider.notifier).registerUser(
        const AuthEntity(
            fullname: 'hi', username: 'hi', email: 'hi', password: 'hi'));
